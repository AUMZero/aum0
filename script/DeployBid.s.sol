...
/// Wallet-edition venue, widened. USDG quote plus every asset on Hood Chain
/// that has both an official feed and a real USDG pool: twenty six in all,
/// each pinned to its deepest fee tier, measured on-chain 2026-09-04.
///
/// The point of the widening is what the last eleven bring with them. SGOV is
/// a treasury bill, so a portfolio can finally hold something that is not a
/// stock. QQQ is the nasdaq. SLV is silver and USO is oil. A sixty forty, or
/// anything with a real bond leg, becomes expressible here for the first time.
///
/// Frozen at deploy forever, like every venue before it.
contract DeployBid is Script {
    ...
    function run() external {
        ...
        AUM0Bid aum = _deploy();
        vm.stopBroadcast();
        console2.log("AUM0 bid edition deployed at:", address(aum));
    }

    function deployForTest() external returns (AUM0Bid) {
        return _deploy();
    }

    function _deploy() internal returns (AUM0Bid) {
