...
/// v2 venue: USDG quote + the fifteen Hood Chain stocks that have both a live
/// feed and a real USDG pool. Each stock is pinned to its deepest sensible fee
/// tier, measured on-chain 2026-08-31. The venue is frozen at deploy forever.
contract DeployV2 is Script {
    ...
    function run() external {
        ...
        AUM0 aum = _deploy();
        vm.stopBroadcast();
        console2.log("AUM0 v2 deployed at:", address(aum));
    }

    function deployForTest() external returns (AUM0) {
        return _deploy();
    }

    function _deploy() internal returns (AUM0) {
        ...
        return new AUM0(ROUTER, USDG, 6, 7 days, tokens, feeds, fees, decs);
    }
