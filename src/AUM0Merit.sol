...
contract AUM0Merit {
    ...
    struct Account {
        ...
        // The follower's own high water mark, set to the policy's index on the
        // day they signed. They never pay the author for a rise that happened
        // before they arrived, and never pay twice for the same rise.
        uint256 meritHwm;
        uint16 minDriftBps;
        uint16 bandBps;
        uint128 bountyQuote;
    ...
    struct Policy {
        ...
        uint32 followers;
        bool retired;
        // The covenant. Limits the author carves into the policy on the day it
        // is published and can never loosen, not by revision, not ever. A
        // revision that would break one of these is refused by the chain.
        uint16 maxAssetBps;   // no single holding may exceed this. 0 means no ceiling
        uint16 minCashBps;    // cash may never fall below this
        // The record. An index of what the published allocation itself did,
        // computed from the weights the author carved and the official feeds,
        // so it can never be restated, redated or dressed up for a reporting
        // day. It starts at one and moves with the market from then on.
        uint256 index;
        uint64[] lastPx;      // the feeds when the index was last brought current
    }
    ...
    /// The limits an author binds themselves to, on the day they publish.
    event Covenant(uint256 indexed id, uint16 maxAssetBps, uint16 minCashBps);
    /// The published allocation's own record, brought current.
    event Merit(uint256 indexed id, uint256 index);
    ...
    uint256 internal constant ONE = 1e18;   // the index starts here and moves from there
    ...
    function publish(
        ...
        if (minCashBps > 10000) revert BadParams();
        _covenant(fromBps, maxAssetBps, minCashBps);
        if (toBps.length != 0) _covenant(toBps, maxAssetBps, minCashBps);
        _policies.push(Policy({
            ...
            followers: 0,
            retired: false,
            maxAssetBps: maxAssetBps,
            minCashBps: minCashBps,
            index: ONE,
            lastPx: new uint64[](0)
        }));
        id = _policies.length;
        _snap(_policies[id - 1], _priceAll());
        emit PolicyPublished(id, msg.sender, royaltyBps);
        emit Covenant(id, maxAssetBps, minCashBps);
    }
    ...
    function revise(
        ...
        _checkAnchors(fromBps, toBps, start, end);
        // The covenant was carved by this author on the day they published and
        // does not bend for them now.
        _covenant(fromBps, p.maxAssetBps, p.minCashBps);
        if (toBps.length != 0) _covenant(toBps, p.maxAssetBps, p.minCashBps);
        // The record is brought current under the old weights before the new
        // ones take effect, so a revision can never rewrite what already happened.
        _settle(p, _priceAll());
        p.targetBps = fromBps;
        p.endBps = toBps;
        p.glideStart = start;
    ...
    function follow(uint256 id, uint16 minDriftBps, uint16 bandBps, uint128 bountyQuote) external {
        ...
        a.meritHwm = p.index;   // you arrive at today's mark, never behind it
        delete a.targetBps;
        delete a.endBps;
        a.glideStart = 0;
        a.glideEnd = 0;
    ...
    /// The covenant. An author writes these limits into the policy on the day
    /// they publish and can never loosen them, so a follower reads once and is
    /// safe from every revision that follows. On Wall Street the same promise
    /// lives in a prospectus, where the only thing enforcing it is a person
    /// reading the filing afterwards.
    function _covenant(uint16[] calldata w, uint16 maxAssetBps, uint16 minCashBps) internal pure {
        if (w[0] < minCashBps) revert CovenantBroken();
        if (maxAssetBps == 0) return;
        for (uint256 i = 1; i < w.length; ++i) if (w[i] > maxAssetBps) revert CovenantBroken();
    }

    /// Write down where the feeds stand, so the next settlement knows what the
    /// allocation was measured from.
    function _snap(Policy storage p, uint256[] memory prices) internal {
        uint256 n = prices.length;
        if (p.lastPx.length != n) {
            delete p.lastPx;
            for (uint256 i; i < n; ++i) p.lastPx.push(uint64(prices[i]));
        } else {
            for (uint256 i; i < n; ++i) p.lastPx[i] = uint64(prices[i]);
        }
    }

    /// Bring the policy's record current. The index is what the published
    /// allocation itself did between the last settlement and this block, using
    /// the author's own weights and the chain's official feeds. There is no
    /// reporting date to dress up for and nothing here can be restated.
    function _settle(Policy storage p, uint256[] memory prices) internal {
        if (p.lastPx.length != prices.length) { _snap(p, prices); return; }
        uint16[] memory w = _interp(p.targetBps, p.endBps, p.glideStart, p.glideEnd);
        uint256 growth;
        for (uint256 i; i < w.length; ++i) {
            if (w[i] == 0) continue;
            uint256 last = p.lastPx[i];
            if (last == 0) continue;
            growth += (uint256(w[i]) * ONE * prices[i]) / (10000 * last);
        }
        if (growth != 0) p.index = (p.index * growth) / ONE;
        _snap(p, prices);
    }
    ...
    /// The royalty on one paid bounty, and who it belongs to. An author is
    /// paid only when the allocation they published stands above the mark it
    /// held the last time this follower paid them, and every follower carries
    /// their own mark from the day they signed. So nobody pays for a rise that
    /// happened before they arrived, nobody pays twice for the same rise, and
    /// an author who is wrong collects nothing at all, for as long as they are
    /// wrong. The number that decides it is the feeds, which no one can restate.
    function _royaltyOn(Account storage acct, uint256 bounty, uint256[] memory prices)
        internal
        returns (uint256 cut, address author)
    {
        if (acct.policyId == 0 || bounty == 0) return (0, address(0));
        Policy storage p = _policies[acct.policyId - 1];
        author = p.author;
        _settle(p, prices);
        emit Merit(acct.policyId, p.index);
        if (p.index <= acct.meritHwm) return (0, author);   // no new high, no royalty
        acct.meritHwm = p.index;
        cut = (bounty * p.royaltyBps) / 10000;
    }
    ...
    function rebalance(address user, Trade[] calldata trades) external {
        ...
        if (bounty > 0) {
            (uint256 cut, address author) = _royaltyOn(acct, bounty, prices);
            if (cut > 0) {
    ...
    function cross(address seller, address buyer, uint256 asset, uint256 amount) external {
        ...
        _pay(seller, sBefore, sAfter, uint256(sAcct.bountyQuote), prices);
        _pay(buyer, bBefore, bAfter, uint256(bAcct.bountyQuote), prices);
    ...
    function _pay(address user, uint256 before_, uint256 after_, uint256 bountyQuote, uint256[] memory prices) internal {
        uint256 earned = (bountyQuote * (before_ - after_)) / 10000;
        if (earned > 0) {
            (uint256 cut, address author) = _royaltyOn(_accounts[user], earned, prices);
            if (cut > 0) {
                if (!quote.transferFrom(user, author, cut)) revert TransferFailed();
                emit Royalty(author, user, cut);
            }
    ...
    /// What the author bound themselves to, and what their allocation has done
    /// since the day it was published.
    function covenantOf(uint256 id)
        external
        view
        returns (uint16 maxAssetBps, uint16 minCashBps, uint256 index)
    {
        Policy storage p = _policyAt(id);
        return (p.maxAssetBps, p.minCashBps, p.index);
    }

    /// The mark this account last paid its author at.
    function meritHwmOf(address user) external view returns (uint256) {
        return _accounts[user].meritHwm;
    }
